...
import { commentPublish, Detailedblog } from "@/Interfaces/Blogs";
const Blogid = () => {
  ...
  const userSubmit = async (e: any) => {
    ...
    await axios
      .post(`../api/Blogs/${Blogid}`, newcomment)
      .then((res) => {
          ...
          <div className={styles.detailed_blog_container_entire}>
            ...
            <div className={styles.detailed_blog_container_entire_box1}>
              <div className={styles.detailed_blog_container}>
                <p className={styles.detailed_blog_container_date}>
                  {Blogdata.publishDate} | Genmatrix Remedies
                </p>
                <h3>{Blogdata.title}</h3>
                <div className={styles.detailedblog_main_image_div}>
                  <Image
                    src={Blogdata.image}
                    className={styles.detailedblog_main_image}
                    alt=""
                    width={500}
                    height={500}
                  />
              ...
              <div className={styles.detailedblog_main_content_div_entire}>
                <div
                  className={styles.detailedblog_main_content_div}
                  dangerouslySetInnerHTML={{ __html: Blogdata.bodycontent }}
                ></div>
              </div>
              <div className={styles.blog_comment_section}>
                ...
                {pagecomments.length !== 0 ? (
                  ...
                  <p className={styles.no_comments_block}>
                    No comments ! Be first to add a comment
                  </p>
                )}
              </div>
            </div>
            <div className={styles.blogbox_container_sticky}>
              <div className={styles.blogbox_container}>
                ...
                {data.slice(0, 3).map((blog: any) => {
                  return (
                    <div className={styles.blogbox_container_blocks}>
                      <div className={blogadstyles.blogbox_con}>
                        <div className={blogadstyles.blogbox_image}>
                          <Image
                            className={blogadstyles.blogbox_image}
                            src={blog.image}
                            alt=""
                            width={500}
                            height={500}
                        ...
                        <Link
                          href={`/blogs/${blog._id}`}
                          className={blogadstyles.blogbox_heading}
                        >
                          <h3>{blog.title}</h3>
                        </Link>
                        <p className={blogadstyles.content}>
                          {blog.description.slice(0, 150)}...{" "}
                          <Link
                            href={`/blogs/${blog._id}`}
                            className={blogadstyles.blog_viewmore}
                          >
                            Continue reading
                          </Link>
                        </p>
