...
const Addresses = ({addressData}:any) => {
  ...
  const handleSubmit = (e: any) => {
    e.preventDefault();
    axios.post(`/api/users/${session?.user?._id}/addresses`, formData)
      .then((res) => {
      ...
      <div className={styles.container}>
        <div className={styles.block}>
          ...
          <div className={styles.dashboard}>
            <h5>ADDRESSES</h5>
            <button className={styles.add_address} onClick={()=>{setShowForm(true)}}>ADD A NEW ADDRESS</button>
            {showForm?<form className={styles.checkout_form} onSubmit={handleSubmit}>
